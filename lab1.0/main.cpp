...
int main(void)
{
    ...
    TritSet C(5);
    C = !(A&B);
    cout<< bitset<32>(A.data[0]) << endl << bitset<32>(B.data[0])<<endl;
    cout<< bitset<32>(C.data[0])<<endl;
    TritSet A(1);
    A[0] = True;
    A[2] = True;

    A[16] = True;
    cout<< bitset<32>(A.data[0])<<endl << bitset<32>(A.data[1]) << endl<<endl;
    A.shrink();
    A.shrink();
    A.shrink();
    cout << (A[2] == True)<<endl;
    cout<< A.cardinality(Unknown);
